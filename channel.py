...
class YtChannel:
    ...
    def get_live_id(self, browser):
        ...
        try:
            ...
            logger.error("timeout error while polling live status")
            logger.error(str(e))
    ...
    def wait_for_live_id(self):
        ...
        with sync_playwright() as p:
            ...
            while not video_id:
                video_id = self.get_live_id(browser)
                if not video_id:
    ...
    def test(self, video_id: str):
        ...
        while chat.is_alive():
            ...
            logger.info(f"attempted to add {l_count} items. had: {l_err} errors/duplicates")
        logger.info(f"total had {count} items. {err_count} errors/duplicates")
        # success_count = self.conn.insert_comments(list(messages.sync_items()))
